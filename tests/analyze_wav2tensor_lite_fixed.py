...
Fixed version to handle missing planes gracefully in visualization.
"""
...
def visualize_comparison(waveform, original_planes, lite_tensor, lite_metadata, output_prefix, sample_rate, n_fft):
    ...
    # Calculate time axis
    if 'spectral' in original_planes and original_planes['spectral'] is not None:
        ...
        # Fallback if spectral plane is missing
        hop_length = n_fft // 4
        time_frames = lite_tensor.shape[-1]
        duration_sec = time_frames * hop_length / sample_rate
        time_ticks = np.linspace(0, time_frames, 5)
        time_labels = [f"{t:.1f}" for t in np.linspace(0, duration_sec, 5)]
        spec_mag = None
    ...
    if 'spectral' in original_planes and original_planes['spectral'] is not None:
        ...
        axs[0, 1].text(0.5, 0.5, "Spectral Plane Not Available", 
                     ha='center', va='center', transform=axs[0, 1].transAxes)
        axs[0, 1].set_title("Original Spectral Magnitude")
        axs[0, 1].axis('off')
    
    # Plot original harmonic plane
    if 'harmonic' in original_planes and original_planes['harmonic'] is not None:
        ...
        axs[0, 2].text(0.5, 0.5, "Harmonic Plane Not Available", 
                     ha='center', va='center', transform=axs[0, 2].transAxes)
        axs[0, 2].set_title("Original Harmonic Plane")
        axs[0, 2].axis('off')
    
    # ---------------------- Row 2 ----------------------
    # Plot original spatial plane - IPD
    if 'spatial' in original_planes and original_planes['spatial'] is not None and original_planes['spatial'].shape[1] >= 1:
        ...
        axs[1, 0].text(0.5, 0.5, "Spatial IPD Not Available", 
                     ha='center', va='center', transform=axs[1, 0].transAxes)
        axs[1, 0].set_title("Original Spatial - IPD")
        axs[1, 0].axis('off')
    
    # Plot original spatial plane - Energy Panning
    if 'spatial' in original_planes and original_planes['spatial'] is not None and original_planes['spatial'].shape[1] >= 2:
        spatial = original_planes['spatial'].squeeze().detach().cpu().numpy()
        energy = spatial[1]
        im4 = axs[1, 1].imshow(
            energy, 
    ...
    if 'psychoacoustic' in original_planes and original_planes['psychoacoustic'] is not None:
        ...
        axs[1, 2].text(0.5, 0.5, "Psychoacoustic Plane Not Available", 
    ...
    # Add empty slots with text for missing Lite channels
    for i in range(lite_tensor.shape[1], 3):
        axs[2, i].text(0.5, 0.5, f"Lite Channel {i+1} Not Available", 
                     ha='center', va='center', transform=axs[2, i].transAxes)
        axs[2, i].axis('off')
